    ...
    # for max_depth in [200]:
        for ngram_range in [(1, 2), (1, 3)]:
            ...
            # for max_features in [40000]:
                name = "XGBoost(n_iter {0} max_depth {1}) + Count(ngram {2}, max_features {3})".format(n_iter,
                                                                                                       max_depth,
                                                                                                       ngram_range,
                                                                                                       max_features)
                params = {"n_iter": n_iter, "max_depth": max_depth}
                transformer = CountVectorizer(ngram_range=ngram_range, max_features=max_features)
                model = XGboostModel(
                    name,
                    params,
                    transformer
